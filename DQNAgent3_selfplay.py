...
class DQNAgent3(BaseAgent):
    def __init__(self, player, name=None, epsilon=1, epsilon_decay=0.99975,
        ...
        self.current_game_states_p1 = []
        self.current_game_states_p2 = []
    ...
    def move(self, state, player):
        if player == self.player:
            ...
            self.current_game_states_p2.append(copy_state)
            ...
            copy_state = copy.deepcopy(state)
            self.current_game_states_p1.append(copy_state)
            ...
            if np.random.random() < self.epsilon:
                ...
                for action in actions:
                    copy_state = copy.deepcopy(state)
                    copy_state[action] = self.other_player_token
                    input_data = np.dstack((copy_state, np.fliplr(copy_state), -self.player_matrix))
                    value = self.model.predict(np.expand_dims(input_data, axis=0))
                    values.append(value)
                idx = np.argmax(values)
    ...
    def update_memory(self, winner):
        if winner == self.player:
            ...
            for state_self, state_opp in zip(reversed(self.current_game_states_p1), reversed(self.current_game_states_p2)):
